...
# use like this:
# ./start-gretl.sh --docker-image sogis/gretl-runtime:latest [--docker-network NETWORK] --topic-name topicname --schema-dirname schemadirname [--db-name dbname] [taskName...] [--option-name...]
...
while [ $# -gt 0 ]; do
    if [[ $1 == "--docker-image" ]]; then
        ...
        shift
    elif [[ $1 == "--topic-name" ]]; then
        declare topic_name="$2"
        shift
    elif [[ $1 == "--schema-dirname" ]]; then
        declare schema_directory_name="$2"
        shift
    elif [[ $1 == "--db-name" ]]; then
        declare db_name="$2"
        shift
...
# Determine the base directory containing all jobs.
# It's the same directory where the current script resides.
declare jobs_directory=$(dirname $(realpath "$0"))
...
echo "Jobs directory: $jobs_directory"
echo "Topic name: $topic_name"
echo "Schema directory name: $schema_directory_name"
...
docker run -i --rm --name gretl \
    ...
    ${network_string} \
    "$docker_image" "-c" \
        "/usr/local/bin/run-jnlp-client > /dev/null 2>&1;cd /home/gradle/project/topics/$topic_name/$schema_directory_name;$gretl_cmd"
